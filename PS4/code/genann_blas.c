...
double const *genann_run(genann const *ann, double const *inputs) {
   ...
   /* for(h = 0; h < ann->hidden_layers; ++h) {
        ...
        cblas_dgemv(101, 111, m, n, 1.0, w+1, n, i, 1, 0, o, 1);
        for(j = 0; j < m; ++j) {
            double value = act(*o + *w * -1.0);
            *o++ = value;
            w += n + 1;
        }
        i += n;
    ...
    /*m = ann->outputs;
    n = (ann->hidden_layers ? ann->hidden : ann->inputs);
    cblas_dgemv(101, 111, m, n, 1.0, w+1, n, i, 1, 0, o, 1);
    for(j = 0; j < m; ++j) {
        double value = acto(*o + *w * -1.0);
        *o++ = value;
        w += n + 1;
    }*/
...
void genann_train(genann const *ann, double const *inputs, double const *desired_outputs, double learning_rate) {
    ...
    //##########################################################################
        ...
        double delta[ann->hidden];
        int m = ann->hidden;
        int n = (h == ann->hidden_layers-1 ? ann->outputs : ann->hidden);
        cblas_dgemv(101, 111, m, n, 1.0, ww+1, n, dd, 1, 0, delta, 1);
        for(j = 0; j < ann->hidden; j++) {
            *d = *o * (1.0-*o) * delta[j];
            ++d; ++o;
        }
    ...
    /* Train the outputs. */
    {
        ...
        for (j = 0; j < ann->outputs; ++j) {
            double a = *d * learning_rate;
            int n = (ann->hidden_layers ? ann->hidden : ann->inputs);
            *w++ += a * -1.0;
            cblas_daxpy(n, a, i, 1, w, 1);
            ++d;
            w += n;
        }
        // assert(w - ann->weight == ann->total_weights);
